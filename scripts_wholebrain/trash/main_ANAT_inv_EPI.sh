...
# set required paths!
source /home/kleinrl/projects/laminar_fmri/paths_inv_EPI
...
if [ ! -d "$SUBJECTS_DIR/$subjid" ]; then 
  ...
  cp -r $recon_dir ${recon_dir}_post-all
...
cp $ds_dir/ANAT/ANAT_working_ANTs/inv_warped_brainmask.manualedits3.mgz $recon_dir/mri/
cp $recon_dir/mri/inv_warped_brainmask.manualedits3.mgz $recon_dir/mri/brainmask.mgz

cp $ds_dir/ANAT/ANAT_working_ANTs/inv_warped_wm.manualedits2.mgz $recon_dir/mri/
...
cp -r $recon_dir ${recon_dir}_post-pial
...
cp -r $recon_dir ${recon_dir}_post-WM-AR3
