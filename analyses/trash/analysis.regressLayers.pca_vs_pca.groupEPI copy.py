...
def plot_regress_layers(seed, control, glm_inputs, layers, params): 
    ...
    plt.plot(glm_inputs['seed - control'], label='seed - control')
...
def regress_across_columns(job):
    ...
    plot_dir    ='plots/regressLayers_columns'+layer4EPI_base
    ...
    coef        = plot_regress_layers(seed, control, glm_inputs, regressors, params)
...
for seed_pca in [seed_epi_mean]: 
    for control_pca in [control_epi_mean]: 
        ...
        glm_inputs = preproc(preproc(seed_pca) - preproc(control_pca))
        
        d = {'seed':seed_pca, 'control': control_pca, 'glm_inputs': glm_inputs}
        inputs.append(d)

glm_inputs = {}
glm_inputs['seed'] = seed_epi_mean
glm_inputs['control'] = control_epi_mean
glm_inputs['seed - control'] = seed_epi_mean - control_epi_mean
...
for e_i in range(len(EPIs)):
    for d_i in range(len(inputs)): 
        ...
        regress_across_columns(ind_jobs[1])
        '''
