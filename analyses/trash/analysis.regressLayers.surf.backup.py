...
def main(EPI_to_run, n_cores=10):
    ...
    plot_dir_top    = '/regressLayers_roi/'
    ...
    #EPI_to_run_base = EPI_to_run.split('/')[-1].split('.')[0]
    ...
    for d_i in range(len(inputs)): 
        ...
        layer4EPI_base = EPI_to_run.split('/')[-1].split('.')[0]
        layer4EPI      = recon_dir + '/LAYNII_'+layer4EPI_base 

        plot_dir        = main_dir + plot_dir_top + layer4EPI_base + "_i-{}".format(d_i)
        os.makedirs(plot_dir, exist_ok=True) 

        surf_dir = SUBJECTS_DIR + '/' + layer4EPI_base + '/surf'

        rh_vert_layers = glob(surf_dir + '/*movie*rh*white.surf-fwhm-2.fsaverage.mgh').sort()
        lh_vert_layers = glob(surf_dir + '/*movie*lh*white.surf-fwhm-2.fsaverage.mgh').sort()
        ...
        try: 
            ...
            with  open('{}/completed_jobs.{}.i_{}.pkl'.format(plot_dir, layer4EPI_base, d_i), "wb" )  as f: 
...
if __name__ == "__main__":
    ...
    print("   ")
    print('RUNNING: {}'.format(EPI_to_run))
    print('using n_cores: {}'.format(n_cores))


    main(EPI_to_run, n_cores=n_cores)
