...
def regress_across_columns(job):
    ...
    coef        = plot_regress_layers(seed, control, glm_input, regressors, params)
...
control_pcas    = get_pcas(control_epi.T, num_timeseries=4)
...
inputs = []
for seed_pca in seed_pcas: 
    for control_pca in control_pcas: 
        glm_input = preproc(preproc(seed_pca) - preproc(control_pca))
        
        d = {'seed':seed_pca, 'control': control_pca, 'glm_input': glm_input}
        inputs.append(d)
    ...
    for epi in EPIs: 
        ...
        for c in column_ids: 
            ...
            job = {'inds': inds, 'cs': cs, 'ls': ls, 
                    'seed': seed, 'control': control, 'glm_input': glm_input, 
