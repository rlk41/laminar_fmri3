...
# copied from analysis.layer_regress.scaled
...
def get_layer_vert_matrix(vert_ids, layers):
    '''
    vert_ids = L_V1_verts
...
def main(EPI_to_run):
    ...
    columns_rh_surf_data = columns_rh_surf_file.get_data()
    columns_lh_surf_data = columns_lh_surf_file.get_data()
    ...
    for e_i in range(len(EPIs)):
        for d_i in range(len(inputs)): 
            ...
            # SURFACES
            surf_dir = SUBJECTS_DIR + '/' + layer4EPI_base + '/surf'

            rh_vert_layers = glob(surf_dir + '/*movie*rh*white.fsaverage.mgh')
            lh_vert_layers = glob(surf_dir + '/*movie*lh*white.fsaverage.mgh')


            L_V1_layers_data = get_layer_vert_matrix(L_V1_verts, lh_vert_layers)
            R_V1_layers_data = get_layer_vert_matrix(R_V1_verts, rh_vert_layers)
            L_MT_layers_data = get_layer_vert_matrix(L_MT_verts, lh_vert_layers)
            ...
            completed_jobs = parallelize(ind_jobs[0:40], regress_across_columns, n_cores=40)
