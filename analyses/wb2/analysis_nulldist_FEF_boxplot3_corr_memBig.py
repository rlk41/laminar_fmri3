...
def get_plots(base_dir, k, fwhm=7): 
    ...
    for uc in unq_roi_col: 
        ...
        for j in range(1, len(null_layer_data_concat)):
            ...
            ax.errorbar(p1, np.mean(null_layer_data_concat[g]), yerr=np.std(null_layer_data_concat[g]), fmt='ok', lw=3, color='blue')
            #ax.errorbar(p2, np.mean(mean_null_layer_data[g]), yerr=np.std(mean_null_layer_data[g]),  fmt='ok', lw=3, color='green')
            ax.errorbar(p3, np.mean(emp_layer_data[g]), yerr=np.std(emp_layer_data[g]),  fmt='ok', lw=3, color='red')
            ...
            #mi, ma, me = np.min(mean_null_layer_data[g]), np.max(mean_null_layer_data[g]), np.mean(mean_null_layer_data[g])
            ...
            ax.errorbar(p3, me,[[ me - mi],[ma - me]] ,  fmt='.k', ecolor='gray', lw=1)
            ...
            try:
                ...
                if w.pvalue < p_thresh: 
                    ...
                    plt.annotate('*', xy=(p3, y_max_annot ), color='blue', fontsize=7)
            ...
            plt.annotate('{}'.format(len(null_layer_data_concat[g])), xy=(p2, y_min_annot*.95), color='blue', fontsize=7)
            #plt.annotate('{}'.format(len(mean_null_layer_data[g])), xy=(p1, y_min_annot), color='green', fontsize=7)
            plt.annotate('{}'.format(len(emp_layer_data[g])), xy=(p2, y_min_annot*.9), color='red', fontsize=7)
        ...
        hG, = plot([1,1],'g-')
        hR, = plot([1,1],'r-')
        legend((hB, hG, hR),('NULL', 'NULL_mean','VASO'), loc='upper left', prop={'size': 6})
