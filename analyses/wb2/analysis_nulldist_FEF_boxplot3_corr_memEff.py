...
def get_plots(base_dir, k=None, c=None, fwhm=7, y_label='', plot_dir=None): 
    ...
    nulls_3d            = glob.glob(base_dir+"/fsl_feat_*NULL*/mean/inv_corr.fwhm{}.nii.gz".format(fwhm)) #fwhm7.L2D.columns_ev_30000_borders.downscaled2x_NN.nii.gz")
    feat_3d             = glob.glob(base_dir+"/fsl_feat_1010.L_FEF_pca10/mean/inv_corr.fwhm{}.nii.gz".format(fwhm)) #L2D.columns_ev_30000_borders.downscaled2x_NN.nii.gz")
    feat_3d             = feat_3d[0]
    ...
    for uc in unq_roi_col: 
        ...
        for j in range(1, len(null_layer_data_concat)):
            ...
            # ax.errorbar(p1, np.mean(null_layer_data[g]), yerr=np.std(null_layer_data[g]), fmt='-o', color='blue')
            # ax.errorbar(p2, np.mean(mean_null_layer_data[g]), yerr=np.std(mean_null_layer_data[g]), fmt='-o', color='green')
            # ax.errorbar(p3, np.mean(emp_layer_data[g]), yerr=np.std(emp_layer_data[g]), fmt='-o', color='red')

            ax.errorbar(p1, np.mean(null_layer_data_concat[g]), yerr=np.std(null_layer_data_concat[g]), fmt='o', lw=3, color='blue')
            #ax.errorbar(p2, np.mean(mean_null_layer_data[g]), yerr=np.std(mean_null_layer_data[g]),  fmt='ok', lw=3, color='green')
            ax.errorbar(p2, np.mean(emp_layer_data[g]), yerr=np.std(emp_layer_data[g]),  fmt='o', lw=3, color='red')
            ...
            try:
                ...
                if w.pvalue < p_thresh: 
                    plt.annotate('*', xy=(p1, y_max_annot ), color='black', fontsize=10)
                        # (p1+p2)/2
            ...
            try:
                w = scipy.stats.wilcoxon(emp_layer_data[g], null_layer_data_concat[g], alternative='greater' )
                if w.pvalue < p_thresh: 
                    plt.annotate('*', xy=(p1, y_max_annot*1.1 ), color='grey', fontsize=10)
                        # (p1+p2)/2
                if w.pvalue < p_thresh/bon_corr: 
                    plt.annotate('**', xy=(p1, y_max_annot*1.1 ), color='grey', fontsize=10)
            ...
            try:
                w = scipy.stats.wilcoxon(emp_layer_data[g], null_layer_data_concat[g] )
                if w.pvalue < p_thresh: 
                    plt.annotate('*', xy=(p1, y_max_annot*1.3 ), color='purple', fontsize=10)
                        # (p1+p2)/2
                if w.pvalue < p_thresh/bon_corr: 
                    plt.annotate('**', xy=(p1, y_max_annot*1.3 ), color='purple', fontsize=10)
        ...
        hG, = plot([1,1],'g-')
        hR, = plot([1,1],'r-')
        legend((hB, hG, hR),('NULL', 'VASO'), loc='upper left', prop={'size': 6})
        ...
        plt.savefig(plot_dir+"/{}.corr.SD.box8.png".format(save_tit))
        ...
        for j in range(1, len(null_layer_data_concat)):
            ...
            ax.errorbar(p1, np.mean(null_layer_data_concat[g]), yerr=np.std(null_layer_data_concat[g])/np.sqrt(len(null_layer_data_concat[g])), fmt='ok', lw=3, color='blue')
            ax.errorbar(p2, np.mean(emp_layer_data[g]),         yerr=np.std(emp_layer_data[g])/np.sqrt(len(emp_layer_data[g])),  fmt='ok', lw=3, color='red')
            ...
            try:
                ...
                if w.pvalue < p_thresh: 
                    ...
                    plt.annotate('*', xy=(p1, y_max_annot ), color='black', fontsize=7)
                    # (p1+p2)/2
                if w.pvalue < p_thresh/bon_corr: 
                    #plt.annotate('** {0:.1E}'.format(w.pvalue), xy=(p3, y_max_annot ), color='blue', fontsize=7)
                    plt.annotate('**', xy=(p1, y_max_annot ), color='black', fontsize=7)
        ...
        plt.savefig(plot_dir+"/{}.corr.SE.box8.png".format(save_tit))
        ...
        hG, = plot([1,1],'g-')
        hR, = plot([1,1],'r-')
        legend((hB, hG, hR),('NULL','VASO'), loc='lower right', prop={'size':6})
        hB.set_visible(False)
        hG.set_visible(False)
        hR.set_visible(False)
        ...
        plt.savefig(plot_dir+"/{}.corr.bp.box8.png".format(save_tit))
...
if __name__ == "__main__":
    ...
    get_plots(base_dir, k=k,c=c, fwhm=fwhm, plot_dir=plot_dir)
