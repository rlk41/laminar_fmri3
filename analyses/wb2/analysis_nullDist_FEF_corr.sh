...
swarm_merged_feat_100000=$swarm_merged_dir/swarm_merged_100000.feat
...
#dep_merged_feat=$(swarm -f /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr/fsl_feat_1010.L_FEF_pca10/swarm/swarm.feat -b 10 -g 5 -t 1 --job-name feat_100k --logdir $swarm_merged_dir --time 00:03:00 )
...
for out_dir in $analysis_dir/fsl_feat_*; do 
    ...
    echo "run_laminar_permute_post.sh -d $out_dir -f corr.nii.gz" | tee -a $swarm_merged_dir/swarm_merged.post 
...
dep_merged_post=$(swarm -f $swarm_merged_dir/swarm_merged.post -g 10 --job-name swarm_post_merged --logdir $swarm_merged_dir \
--time 02:00:00)
...
for r in ${rois[@]}; do 
for s in 1 ; do 
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_corr.py --base_dir $analysis_dir --k $r --fwhm $s " | tee -a $swarm_plots
done 
done 
cat $swarm_plots | wc 
...
for r in ${rois[@]}; do 
for s in 1 3 5 ; do 
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_corr_memEff.py --base_dir $analysis_dir  --k $r --fwhm $s " | tee -a $swarm_plots
...
plot_cmp_dir=$analysis_dir/L_VIP_10070-compare
mkdir $plot_cmp_dir
...
for s in 1 3 5 7 8 ; do 
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py --type corr --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr  --c 10070 --k L_VIP --fwhm $s --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py --type regress --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF   --c 10070 --k L_VIP --fwhm $s --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
...
rois=(L_FEF L_LIPv L_VIP L_7PC L_V4t L_V4 L_V2 L_V3 L_V1 L_MST L_MT
    L_TF L_TE1a L_TE1p L_TE2a L_TE2p L_FST L_7PL )
...
plot_cmp_dir=$analysis_dir/L_VIP_10070-compare
plot_cmp_dir=$analysis_dir/FEF_for_layerClub-compare

plot_cmp_dir=$analysis_dir/FEF_for_layerClub-ALL-compare
mkdir $plot_cmp_dir
...
for r in ${rois[@]}; do 
for s in 1 3 5 7 8 ; do 
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
        --type corr --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr \
        --k $r --fwhm $s --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
    --type regress --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF  \
     --k $r --fwhm $s --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
done
done 
...
for s in 1 3 5 7 8 ; do 
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
        --type corr --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr \
        --k $r --fwhm $s --c $c --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
    --type regress --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF  \
     --k $r --fwhm $s --c $c --plot_dir $plot_cmp_dir" | tee -a $swarm_plots
done
...
swarm -f $swarm_plots -g 20 --job-name plots_Col --time 24:00:00 --logdir $analysis_dir/swarm_merged




scp -r cn0852:/data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr/FEF_for_layerClub-compare/* .
scp -r cn0852:/data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr/FEF_for_layerClub-ALL-compare/* .
...
rm $swarm_plots
touch $swarm_plots
plot_cmp_dir=$analysis_dir/FEF_for_layerClub-seedNULL-compare
mkdir $plot_cmp_dir

#nulls=($( find /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_single/fsl_feat_*/mean/inv_pe1.fwhm7.nii.gz -maxdepth 0 -type d -exec echo {} \; ))
...
for r in L_VIP ; do 
for c in 10070 ; do 
    nulls=($(find /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_single/fsl_feat_*/mean/inv_pe1.fwhm$s.nii.gz ))

    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
        --type corr --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF_corr \
        --k $r --fwhm $s --c $c --plot_dir $plot_cmp_dir --nulls ${nulls[@]} " | tee -a $swarm_plots
    echo "python /home/kleinrl/projects/laminar_fmri/analyses/analysis_nulldist_FEF_boxplot3_both_memEff.py \
    --type regress --base_dir  /data/NIMH_scratch/kleinrl/analyses/nullDist_pca10_FEF  \
     --k $r --fwhm $s --c $c --plot_dir $plot_cmp_dir --nulls ${nulls[@]}  " | tee -a $swarm_plots
done
...
for out_dir in $analysis_dir/fsl_feat_*/DAY*/mean ; do 
    ...
    for smoothing in ${s[@]};do 
    echo "L2D.job.sh $out_dir $smoothing" | tee -a $swarmL2D
    done 
