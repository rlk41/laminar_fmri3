...
analysis_dir="$my_scratch/analyses/seedLayers_FEF"
...
rois=($(ls $roi_dir/*.{csf,mid,wm}.nii.gz))

rois=($(ls $roi_dir/layered_rois/13707.22746.27431.17722.{csf,mid,wm}.nii.gz))
...
#     echo "run_laminar_permute_swarm.sh -r $roi_path -p 0 -o $out_dir -X -A" | tee -a $swarm_submit
...
for out_dir in $analysis_dir/fsl_feat_*; do 
    ...
    echo "L2D.job.sh $dir $smoothing" | tee -a $swarm_merged_L2D 
    done 
...
zstats=($(ls *.feat/stats/zstat1.nii.gz))
3dMean -prefix zstats1.mean.nii.gz ${zstats[@]} -overwrite 
