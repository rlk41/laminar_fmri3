...
analysis_dir="$my_scratch/analyses/wb3/A1_SNR"
...
for batch_num in `seq 0 50 5`; do
for per_batch in `seq 0 10`; do 


    roi_path=${rois[0]} 

    out_dir=$analysis_dir/fsl_feat_AVE_rand_batch_$per_batch

    mkdir -p $out_dir 

    echo "run_swarm_wb3_permute_a1-1.sh -r $roi_path -p 0 -d $batch_num -o $out_dir -A -X" | tee -a $swarm_submit
...
dep_merge_feat_and_run=$( swarm -f $swarm_merge_and_run -g 10 --dependency=afterok:$dep_setup --job-name merge-f-run)
