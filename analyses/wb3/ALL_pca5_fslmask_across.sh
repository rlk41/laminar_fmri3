...
analysis_dir="$my_scratch/analyses/wb3/ALL_pca5_fslmask_across"
...
do    
    ...
    echo "run_swarm_wb3.sh -r $roi_path -p 5 -o $out_dir -X" | tee -a $swarm_submit
...
dep_merged_feat=$(swarm -f $swarm_merged_feat -g 12 -t 1 --job-name feat_merged --logdir $swarm_merged_dir --time 10:00:00 )
...
for out_dir in $analysis_dir/fsl_feat_*; do 
    ...
    # for dir2 in $out_dir/*pca_*/*.feat; do
    # for smoothing in ${s[@]};do 
    # #echo $dir2 
    # echo "L2D.job.wb2.sh $dir2 $smoothing thresh_zstat1.nii" | tee -a $swarm_merged_dir/swarm_merged.L2D 
    # done
    # done 
...
# LN2_todataframe.py \
# --input "/data/NIMH_scratch/kleinrl/analyses/wb3/LANG_pca5_fslmask_AVE/fsl_feat_1140.L_TPOJ2_pca10/mean/thresh_zstat1.fwhm5.nii.gz" \
# --columns  "/data/kleinrl/Wholebrain2.0/ANAT/ANAT_working_recon-all/ANAT_mri_make_surf/multiAtlasTT/hcp-mmp-b/warped_hcp-mmp-b_rmap.nii" \
