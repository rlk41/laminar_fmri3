...
analysis_dir="$my_scratch/analyses/wb3/FEF_aim1/all_auto_iterNULL_newFEAT"
...
for epi_num in 1 5 10 15; do 
    ...
    echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -s $epi_num -o $out_dir -i $imp_type -A -X -B" | tee -a $swarm_submit
    echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -s $epi_num -o $out_dir_null -i $imp_type -A -X -B" | tee -a $swarm_submit
...
feats_to_merge_v2=($(find $analysis_dir  -name swarm.feat_v2 ))
echo ${#feats_to_merge[@]}
...
dep_merged_feat=$(swarm -f $swarm_merged_feat -g 30 -t 1 --job-name feat_merged --logdir $swarm_merged_dir --time 00:10:00 )
