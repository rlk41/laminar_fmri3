...
analysis_dir="$my_scratch/analyses/wb3/L_FEF_PERMUTE_FSLFEAT_QUICK"
...
do    
    ...
    echo "run_swarm_wb3_permute.sh -r $roi_path -p 5 -o $out_dir -X" | tee -a $swarm_submit
...
dep_setup=$(swarm -b 10 -f $swarm_submit -g 10 --job-name setup )


dep_merge_feat_and_run=$( swarm -f $swarm_merge_and_run -g 10 --job-name merge-f-run  --dependency=afterok:$dep_setup)


dep_create_postmerge_L2D=$( swarm -f $post_merge_L2D_job  -g 15 --job-name post_merge-L2D --dependency=afterok:$dep_merge_feat_and_run )
