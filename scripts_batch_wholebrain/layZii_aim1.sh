...
while getopts 'r:p:s:d:i:GN' flag; do
  case "${flag}" in
    r) roi_list="${OPTARG}" ;;
    p) pca_num="${OPTARG}" ;;
    s) epis_to_use="${OPTARG}" ;;
    d) analysis_dir="${OPTARG}" ;;
...
echo "PARAMETERS:"
echo "    "
echo "rois_list:      $roi_list"
echo "rois:           $rois"
echo "                "
echo "pca_num:        $pca_num"
...
do    
    ...
    echo "pca $pca_num perm $epis_to_use roi $roi_path fsl_feat" | tee -a $out_dir/config.txt
    
    if [[ G -eq 1 ]]; then 

      if [[ N -eq 1 ]]; then 
        echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -o $out_dir -i $implementation  -G -N -A -X" | tee -a $swarm_submit
      else
        echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -o $out_dir -i $implementation  -G -A -X" | tee -a $swarm_submit
      fi 
    
    elif [[ G -eq 0 ]]; then 
    
      if [[ N -eq 1 ]]; then 
        echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -s $epis_to_use -o $out_dir -i $implementation  -N -A -X" | tee -a $swarm_submit
      else
        echo "run_swarm_wb3_permute.sh -r $roi_path -p $pca_num -s $epis_to_use -o $out_dir -i $implementation  -A -X" | tee -a $swarm_submit
      fi 
    
    else
      echo "G flag not 1 or 0 "
    ...
    #echo "run_swarm_wb3_permute_grandmean.sh -r $roi_path -p $pca_num -s 1 -o $out_dir -A -X" | tee -a $swarm_submit
