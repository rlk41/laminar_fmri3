  ...
  if [ $num_pcas -eq 0 ]; then 
    ...
    for e_num in $EPI_tocompare; do 
      ...
      if [ $C -eq 1 ]; then 
        ...
        echo "export NOBATCH=true ; OPENBLAS_NUM_THREADS=1; fsl_feat.job.sh $e $timeseries_1D $out_dir_2D $fsl_feat_mask" >> $swarm_file 
      fi 
    ...
    for t in ${ts[@]:0:$num_pcas}; do 
      for e_num in $EPI_tocompare; do 
        ...
        if [ $C -eq 1 ]; then 
          ...
          echo "export NOBATCH=true ;  OPENBLAS_NUM_THREADS=1; fsl_feat.job.sh $e $t $out_dir_2D $fsl_feat_mask" >> $swarm_file 
        fi 
