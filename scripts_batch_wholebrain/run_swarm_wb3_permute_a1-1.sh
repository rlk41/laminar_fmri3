...
while getopts 'r:p:o:t:d:XANC' flag; do
  case "${flag}" in
    r) roi_path="${OPTARG}" ;;
    p) num_pcas="${OPTARG}" ;;
    o) out_dir="${OPTARG}" ;;
...
3dMean -prefix $out_dir/rand_sample.nii.gz ${selected_EPIs[@]}




EPIs=($out_dir/rand_sample.nii.gz)
EPI_total=${#EPIs[@]}
  ...
  if [ $num_pcas -eq 0 ]; then 
    ...
    for e_num in $EPI_tocompare; do 
      ...
      if [ $C -eq 1 ]; then 
        ...
        echo "export NOBATCH=true ; fsl_feat.job.sh $e $timeseries_1D $out_dir_2D $fsl_feat_mask" >> $swarm_file 
      fi 
