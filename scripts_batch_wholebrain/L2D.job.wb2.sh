...
layers1="/data/NIMH_scratch/kleinrl/gdown/sub-02_layers.nii"
layers2="/data/kleinrl/Wholebrain2.0/ANAT/ANAT_working_recon-all/ANAT_mri_make_surf/LAYNII_2/layers/grow_leaky_loituma/equi_volume_layers_n10.nii"
...
if [ ! -f $file_base.fwhm$smoothing.l1.nii.gz ]; then 
    echo "    SMOOTHING"

    LN_LAYER_SMOOTH -layer_file $layers1 \
    -input $file -FWHM $smoothing \
    -mask \
    -output $file_base.fwhm$smoothing.l1.nii.gz \
    -NoKissing 
else 
...
echo "checking if need to smooth"
if [ ! -f $file_base.fwhm$smoothing.l2.nii.gz ]; then 
    echo "    SMOOTHING"

    LN_LAYER_SMOOTH -layer_file $layers2 \
    -input $file -FWHM $smoothing \
    -mask \
    -output $file_base.fwhm$smoothing.l2.nii.gz \
    -NoKissing 
else 
    echo "    no need to smooth"
fi 
...
# else 
#     echo "    no need to LN2_todataframe.py "
# fi 



## commented out 9/1/2022
