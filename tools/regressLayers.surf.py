...
def main(EPI_seed, EPI_target, pkl_dir, n_cores=10, plot_verts=False):
    ...
    layer4EPI_base      = EPI_seed.split('/')[-1].split('.')[0]
    layer4EPI           = recon_dir + '/LAYNII_'+layer4EPI_base
    seed_control_VASO   = layer4EPI_base
    seed_base = layer4EPI_base
    
    main_dir        = '/data/kleinrl/plots/'
    plot_dir_top    = '/regressLayers_{}/'.format(layer4EPI_base)
    ...
    for d_i in range(len(inputs)): 
        ...
        if rh_vert_layers == [] or lh_vert_layers == []:
            print("missing verts for {}".format(layer4EPI_base))
            continue
        ...
        try: 
            ...
            with  open('{}/completed_jobs.seed_{}.target_{}.i_{}.pkl'.format(pkl_dir, seed_base, target_base, d_i), "wb" )  as f: 
