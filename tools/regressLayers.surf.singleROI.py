...
def plot_regress_layers(seed, control, glm_inputs, layers, params): 
    ...
    for k in ['L_LGN - L_MT', 'L_MT - L_LGN', 'R_V1']:
        ...
        coef = reg.coef_
        coef_zscore = (coef - np.mean(coef))/np.std(coef)
...
def regress_layers(seed, control, glm_inputs, layers, params): 
    ...
    for k in ['L_LGN - L_MT', 'L_MT - L_LGN', 'R_V1']:
        ...
        coef_z = (coef- np.mean(coef, axis=0)) / np.std(coef, axis=0)
...
def plot_group_regress_layers(competed_jobs, plot_path): 
    ...
    coefs_zscore = (coefs - np.mean(coefs))/np.std(coefs)
...
def regress_across_columns(job):
    ...
    coefs, coefs_zscore = plot_regress_layers(seed, control, glm_inputs, regressors, params)


    job['coefs'] = coefs 
    job['coefs_zscore'] = coefs_zscore
...
def main(EPI_seed, EPI_target, pkl_dir, vert_base=None, seed_pca=0, control_pca=0, n_cores=10, plot_verts=False, columns=None):
    ...
    columns='rim_columns10000'
    '''
    if vert_base == None:
        print("need to set vert_base")
        return 
        

    seed_roi_path       = '/rois.thalamic.l3/8109.lh.LGN.nii'
    control_roi_path    = '/rois.hcp/1023.L_MT.nii'
    roi2_roi_path       = '/rois.hcp/2001.R_V1.nii'

    layer4EPI_seed_base      = EPI_seed.split('/')[-1].split('.')[0]
    layer4EPI_seed           = recon_dir + '/LAYNII_'+layer4EPI_seed_base

    layer4EPI_target_base  = EPI_target.split('/')[-1].split('.')[0]
    layer4EPI_target       = recon_dir + '/LAYNII_'+layer4EPI_target_base 

    seed                    = layer4EPI_seed + seed_roi_path 
    control                 = layer4EPI_seed + control_roi_path
    roi2_file               = layer4EPI_seed + roi2_roi_path


    which_seed_pca      = seed_pca
    which_control_pca   = control_pca



    plot_dir_top    = '{}/plots.seed_{}.target_{}'.format(pkl_dir, layer4EPI_seed_base, layer4EPI_target_base)

    reports_dir     = plot_dir_top + '/reports'
    ...
    brain_file               = layer4EPI_seed + '/warped_brainmask.bin.nii'
    ...
    seed_epi_mean       = seed_pcas_ts[which_seed_pca]
    control_epi_mean    = control_pcas_ts[which_control_pca]

    inputs=[]
    glm_inputs = {}
    glm_inputs['seed_pcas']                 = seed_pcas_ts
    glm_inputs['control_pcas']              = control_pcas_ts
    glm_inputs['brain']                     = brain_epi_mean
    ...
    surf_dir = SUBJECTS_DIR + '/' + layer4EPI_target_base + '/surf'

    rh_vert_layers = glob(surf_dir + '/*movie*rh*{}'.format(vert_base)) #white.surf-fwhm-2.vol-fwhm-NA.fsaverage.mgh
    lh_vert_layers = glob(surf_dir + '/*movie*lh*{}'.format(vert_base)) #white.surf-fwhm-2.vol-fwhm-NA.fsaverage.mgh

    rh_vert_layers.sort()
    lh_vert_layers.sort()

    if rh_vert_layers == [] or lh_vert_layers == []:
        print("missing verts for {}".format(layer4EPI_target_base))
        raise ValueError("missing verts for {}".format(layer4EPI_target_base))
 

    # COLUMNS 
    if columns != None:

        columns_rh_surf = SUBJECTS_DIR+'/fsaverage.surf/'+columns+'.rh.white.fsaverage.mgh'  
        columns_lh_surf = SUBJECTS_DIR+'/fsaverage.surf/'+columns+'.lh.white.fsaverage.mgh'

        columns_rh_surf_file = mgh.load(columns_rh_surf)
        columns_lh_surf_file = mgh.load(columns_lh_surf)    

        columns_rh_surf_data = columns_rh_surf_file.get_fdata().astype(int)
        columns_lh_surf_data = columns_lh_surf_file.get_fdata().astype(int)


        for unq_col in np.unique(columns_rh_surf_data):
            if unq_col == 0: 
                continue 
            ...
            ind_col     = np.where(columns_rh_surf_data == unq_col)[0]
            col_verts   = get_layer_vert_matrix(ind_col, rh_vert_layers, normalize=False)

            print(ind_col.shape)
            print(col_verts.shape)

            print(len(rh_vert_layers), col_verts.shape[0])
            if len(rh_vert_layers) != col_verts.shape[0]:
                print('layers wrong - not same number. dimensions off')

            print(ind_col.shape[0], col_verts.shape[1])
            if  ind_col.shape[0] != col_verts.shape[1]:
                print('inds wrong - do not match up')


            col_verts = np.mean(col_verts,1)
            print("mean across col.shape {}".format(col_verts.shape))
        ...
        column_ids  = range(columns_layers_data.shape[1])
        layer_ids   = range(columns_layers_data.shape[0])

        ind_jobs = []         
        for c in column_ids: 
            inds, cs, ls, data = [], [], [], []
            data = columns_layers_data[:,c,:].T
            cs.append(c)

            # TODO add d seed, control, glm_input here 
            job = {'inds': inds, 'cs': cs, 'ls': ls, 'data':data,
                    'seed': seed, 'control': control, 'glm_inputs': glm_inputs, 
                    'layer4EPI_base': layer4EPI_target_base, 
                    'seed_control_VASO': layer4EPI_seed_base,
    ...
    try: 
        ...
        with  open('{}/completed_jobs.seed_{}.target_{}.pkl'.format(pkl_dir, layer4EPI_seed_base, layer4EPI_target_base), "wb" )  as f: 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed_pca', type=int, default=0)
    parser.add_argument('--control_pca', type=int, default=0)
    parser.add_argument('--vert_base', type=str)
    ...
    seed_pca    = args.seed_pca
    control_pca = args.control_pca 
    vert_base   = args.vert_base 
    ...
    main(EPI_seed, EPI_target, pkl_dir, vert_base=vert_base, seed_pca=seed_pca, control_pca=control_pca, 
                n_cores=n_cores, plot_verts=False)
